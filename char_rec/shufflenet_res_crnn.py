...
#import resnet
from . import shufflenet_res as shufflenet 
import sys
...
def readfile(filename):
    ...
    for i in res:
        ...
        if len(i[first_whitespace_idx + 1:]) == 0 or (not is_valid(i[first_whitespace_idx + 1:])) or len(img_name) == 0 :
...
def encode_label(text):
    ...
    #print(text,label_list)
    #label_txt_list.writelines(text +str(label_list)+'\n')
    return label_list
